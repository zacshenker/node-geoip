...
(function() {
var ifile, ifd, sz, recsz, buff, lrecsz, lbuff, rbuff = [], cbuff = [];
...
try {
    ...
    fs.readFileSync(path.join(geodatadir, 'maxmind-country-codes.csv')).toString().split("\n").forEach(function (cbuffItem) {
        var cbuffPart = cbuffItem.toString().split(',');

        if (cbuffPart[0] !== undefined) {
	    var countryCode = (cbuffPart[4] + '').replace(/^"/, '').replace(/"$/, '');
	    var countryName = (cbuffPart[5] + '').replace(/^"/, '').replace(/"$/, '');
            cbuff[countryCode] = countryName;
        }
    });
}
catch(err) {
...
lookup4 = function(ip) {
	var fline=0, floor=lastip, cline=lastline, ceil=firstip, line, locId, cc, ccn, rg, rdata, rgn, city, lat, lon, i;
